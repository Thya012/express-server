...
const handleGoogle = asyncHandler(async (req, res) => {
    ...
    const { data } = await axios.post('https://oauth2.googleapis.com/token', {
        ...
        client_secret: process.env.GOOGLE_CLIENT_SECRE,
...
const showGoogleOAuth = async (req, res) => {
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${process.env.GOOGLE_CLIENT_ID}&redirect_uri=${process.env.GOOGLE_CALLBACK_URL}&response_type=code&scope=profile%20email&access_type=offline`;
